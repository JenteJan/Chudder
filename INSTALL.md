...
> [!IMPORTANT]
> **Server on your local network?** Chrome and Edge block a public website, the hosted build included, from talking to a server that resolves to a private address: a LAN IP, a split-DNS hostname, a Tailscale address. The symptoms are a login that half works, rows that stop after a couple of items and searches that fail, with `ERR_BLOCKED_BY_LOCAL_NETWORK_ACCESS_CHECKS` in the browser console. In Chrome, allow it when the browser asks, or under `chrome://settings/content/localNetworkAccess` for `jentejan.github.io`. Firefox has the same protection and lets the login through before it starts refusing everything else, with `NetworkError when attempting to fetch resource` in the console; there the switch is `network.lna.blocking` in `about:config`, set to `false`. Self-hosting the web build on the same network avoids the question in every browser, because then the site is local too.
