...
# splits the take into frames. Output: out/cap/<name>.mp4, .log (a "T <s> <key>"
...
ffmpeg -v error -y -f gdigrab -framerate 30 -draw_mouse 0 -i "hwnd=$hwnd" -t "$seconds" \
  -vf "crop=trunc(iw/2)*2:trunc(ih/2)*2" -c:v libx264 -preset veryfast -crf 16 -pix_fmt yuv420p "out/cap/$name.mp4" &
recorder=$!
sleep 1.2
