...
set -e
cd "$(dirname "$0")/../out"
D=$(cd ../video && python -c "import render8, render10; print('%.3f' % render8.timeline()[1])")
FADE=$(python -c "print('%.3f' % ($D - 2.5))")
ffmpeg -v error -y -framerate 60 -i out10/%05d.png -ss 87.422 -i music/Werq.mp3 \
  -filter_complex "[1:a]loudnorm=I=-14:TP=-1.5:LRA=11,aresample=48000,afade=t=in:st=0:d=0.6,afade=t=out:st=$FADE:d=2.5[a]" \
...
ffprobe -v error -select_streams v:0 -count_packets -show_entries stream=nb_read_packets -of csv=p=0 chudder_showcase_v10.mp4 \
  | xargs -I{} echo "chudder_showcase_v10.mp4: {} video frames (expect $(python -c "print(round($D * 60))"))"
